...
class Handler(webapp.RequestHandler):
  ...
  def render(self, extra_values={},template_file='base.html'):
    ...
    path = os.path.join(cwd, 'templates', template_file)
...
class LogoutHandler(Handler):
  def get(self):
    self.response.headers.add_header('Set-Cookie','claimed_id=logout; expires=Fri, 5-Oct-1979 08:10:00 GMT' )
    self.redirect("/")
...
class LoginHandler(Handler):
  ...
  """Show the login page."""
  def get(self):
    self.render({"claimed_id":"http://mixi.jp/"},'login.html')
...
class FinishHandler(Handler):
  ...
  def get(self):
    ...
    if response.status == 'success':
      #suRes = sreg.SRegResponse.fromSuccessResponse(response).items()
      suRes = sreg.SRegResponse.fromSuccessResponse(response)
      sreg_data = suRes.items()
      ...
      self.response.headers.add_header(
        'Set-Cookie', 
        'claimed_id=%s; expires=Fri, 31-Dec-2020 23:59:59 GMT' \
          % self.session.claimed_id )
